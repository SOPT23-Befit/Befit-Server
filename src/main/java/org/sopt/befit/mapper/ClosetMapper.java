...
public interface ClosetMapper {
    ...
    void postClosetProduct(@Param("user_idx") final int user_idx, @Param("closetReq") final ClosetReq closetReq);
    ...
    int isCloset(@Param("user_idx") final int user_idx, @Param("closetReq") final ClosetReq closetReq);
    ...
    // 나의 옷장 리스트 조회
    @Select("select cwp.closet_idx, b.name_korean, b.name_english, cwp.name, cwp.image_url, cwp.product_size, cwp.measure\n" +
            "    from brand as b,\n" +
            ...
            "    where cwp.product_category_index = #{category_idx} and cwp.brand_idx = b.idx;")
    List<ClosetListReq> getClosetProduct(@Param("user_idx") final int user_idx,
